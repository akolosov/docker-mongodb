...
if [ -n $MONGODB_REPLICA_SET ]; then
  /usr/bin/mongod --config $MONGOD_CONFIG_FILE --replSet $MONGODB_REPLICA_SET --dbpath $MONGODB_DATA_PATH --port $MONGODB_MAIN_PORT --notablescan --noprealloc --smallfiles $OPTIONS	 	
fi
